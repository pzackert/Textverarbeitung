...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load annotated files list from API
    await initializeAnnotatedFiles();
});

// Helper to fetch annotated files
async function initializeAnnotatedFiles() {
    if (!currentDocState.projectId) {
        // Try getting from URL if not set
        currentDocState.projectId = window.location.pathname.split('/')[2];
    }

    // 1. Load Annotated Files
    try {
        const response = await fetch(`/api/projects/${currentDocState.projectId}/documents?view=annotated`);
        if (!response.ok) return; // Silent fail if API issues

        const data = await response.json();

        // Store annotated file names separately
        window.annotatedFiles = data.documents.map(d => d.filename);

        // Also update global projectFiles if needed
        if (!window.projectFiles) window.projectFiles = [];
        window.projectFiles.push(...window.annotatedFiles);

        // Update UI state
        updateToggleUI();

        console.log('Annotated files loaded:', window.annotatedFiles);
    } catch (e) {
        console.error('Failed to load annotated files:', e);
    }

    // 2. Trigger RAG Ingestion for all docs
    ingestAllDocuments();
}
...
async function ingestAllDocuments() {
    ...
    fileItems.forEach(async (item) => {
        ...
        const iconWrapper = item.querySelector('.file-icon-wrapper');
        if (!iconWrapper) return;

        // Check if already processed (check marker)
        if (iconWrapper.querySelector('.rag-status')) return;

        // Add Loading Spinner (SVG Overlay)
        const loadingSvg = document.createElement('div');
        loadingSvg.className = 'rag-status absolute inset-0 flex items-center justify-center pointer-events-none';
        loadingSvg.innerHTML = `
            <svg class="animate-spin h-8 w-8 text-blue-500 opacity-75" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        iconWrapper.appendChild(loadingSvg);

        try {
            const response = await fetch(`/api/projects/${currentDocState.projectId}/documents/${docId}/ingest`, {
                method: 'POST'
            });
            ...
            if (response.ok) {
                ...
                loadingSvg.innerHTML = `
                    <div class="bg-green-100 rounded-full p-0.5 border border-green-200 shadow-sm transform scale-110">
                        <svg class="h-4 w-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
                        </svg>
                    </div>
                `;
                // Move checkmark to bottom-right corner
                loadingSvg.className = 'rag-status absolute -bottom-1 -right-1 flex items-center justify-center';
            } else {
                // Error State
                loadingSvg.innerHTML = ''; // Remove spinner
            }
        } catch (e) {
            console.error(`Ingestion failed for ${filename}`, e);
            loadingSvg.remove();
        }
    });
}
